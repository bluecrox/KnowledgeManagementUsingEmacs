  ...
  (interactive
   (let ( $operator)
     (list
      ...
      (read-from-minibuffer "File path regex: " (xah-find--get-default-file-extension-regex "txt") nil nil 'dired-regexp-history))))
...
\\{xah-find-output-mode-map}"
  (interactive
   (let ( x-search-str x-replace-str x-input-dir x-path-regex x-write-to-file-p x-fixed-case-search-p x-fixed-case-replace-p x-backup-p )
     ...
     (setq x-path-regex (read-from-minibuffer "File path regex: " (xah-find--get-default-file-extension-regex "txt") nil nil 'dired-regexp-history))
